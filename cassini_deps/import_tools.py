...
def latest_version(cas_deps_dir):
    return max([Version(entry.name) for entry in os.scandir(cas_deps_dir) if validate(entry.name)])
...
class PatchImporter:
    ...
    def __init__(self, version, cas_deps_dir):
        self._modules_before = None
        spec = SimpleSpec('{major}.{minor}.*'.format(major=version.major, minor=version.minor))
        max_compatible = spec.select(Version(entry.name) for entry in os.scandir(cas_deps_dir) if validate(entry.name))
        ...
        self.path = str(cas_deps_dir / str(max_compatible))
